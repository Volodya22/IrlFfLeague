...
namespace IrlFfLeague.Web.Infrastructure
{
    public static class ReflectionHelper
    {
        ...
        public static bool HasPropertyWithAttribute(object instance, Type attributeType)
        {
            ...
            else
            {
                foreach (var property in properties)
                {
                    var propType = property.PropertyType;
                    //skip system types
                    if (!propType.IsPrimitive && !propType.Namespace.StartsWith("System") && !instance.GetType().IsGenericType)
                    {
                        var value = property.GetValue(instance);
                        if (value != null)
                        {
        ...
        public static List<PropertyInfo> GetAllStringProperties(Type type)
        {
            return type
